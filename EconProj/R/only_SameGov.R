...
f1 <- as.formula(paste("df$`II.3: Centrally Sponsored Schemes` ~", paste0("df$`",names(df)[c(5)],"`",collapse = "+")))
f2 <- as.formula(paste("df$`II.2: Central Plan Schemes` ~", paste0("df$`",names(df)[c(5)],"`",collapse = "+")))
f3 <- as.formula(paste("df$`I.6: WMA from RBI` ~", paste0("df$`",names(df)[c(5)],"`",collapse = "+")))
f4 <- as.formula(paste("df$`II.D.5.c.1: GST compensation` ~", paste0("df$`",names(df)[c(5)],"`",collapse = "+")))
